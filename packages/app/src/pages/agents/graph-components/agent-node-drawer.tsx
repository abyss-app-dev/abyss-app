    ...
    return (
        <div className="flex flex-row overflow-hidden h-[100vh]">
            <Sidebar className="bg-[#0e0e0e] border-l border-background-600" title="Agent nodes" width={300}>
                ...
                <SidebarButton label="On Chat Message" icon={MessageCircle} onClick={() => onAddNode(Nodes.OnThreadMessage.getDefinition())} />

                <SidebarSection title="Threads" />
                <SidebarButton label="Add System Message" icon={MessageCircle} onClick={() => onAddNode(Nodes.AddSystemMessageToThread.getDefinition())} />
                <SidebarButton label="Add Agent Message" icon={MessageCircle} onClick={() => onAddNode(Nodes.AddAgentMessageToThread.getDefinition())} />
                <SidebarButton label="Add Human Message" icon={MessageCircle} onClick={() => onAddNode(Nodes.AddHumanMessageToThread.getDefinition())} />

                <SidebarSection title="Tools" />
                <SidebarButton label="Reference Toolset" icon={CircleDot} onClick={() => onAddNode(Nodes.ConstToolset.getDefinition())} />
                <SidebarButton label="Set Thread Tools" icon={HammerIcon} onClick={() => onAddNode(Nodes.SetToolsInThread.getDefinition())} />
                ...
                <SidebarButton label="Invoke Model" icon={Sparkles} onClick={() => onAddNode(Nodes.InvokeModelAgainstThread.getDefinition())} />
            </Sidebar>
        </div>
    );
}
